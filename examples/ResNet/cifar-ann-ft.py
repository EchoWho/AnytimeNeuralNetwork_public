...
class Model(ModelDesc):
    ...
    def _build_graph(self, inputs):
        ...
        if SAMLOSS > 0:
            ...
            tf.summary.scalar("select_idx", select_idx)
            for i in range(ls_K):
        ...
        with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='NCHW'):
            ...
            wd_w = 0 #tf.train.exponential_decay(0.0002, get_global_step_var(),
                     #                         480000, 0.2, True)
            wd_cost = 0
            cost = 0
            unit_idx = -1
...
def get_config():
    ...
    if SAMLOSS > 0:
        ...
        if SAMLOSS == 3:
            ...
            # create training scheudle and loss selector for forward thinking
            ...
            # Fixed schedule
            online_learn_cb = ScheduledHyperParamSetter('select_idx', ls_ft_schedule)
        else:    
            gamma = EXP3_GAMMA
            if SAMLOSS == 1:
